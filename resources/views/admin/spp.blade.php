...
@section('title', 'Kelola Pembayaran — RA Al Kautsar')
@section('header_title', 'Pembayaran — Kelola Pembayaran')
...
<div class="space-y-8" x-data="hashTabs('tagihan')">
    ...
    {{-- Tab bar: Tagihan SPP Bulanan → Bukti SPP (selaras daftar-ulang) --}}
    <div class="flex gap-2 border-b border-slate-100">
        <button @click="setTab('tagihan')" :class="tab === 'tagihan' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-400 hover:text-slate-600'"
                class="px-4 py-3 border-b-2 font-bold text-xs transition-colors">Tagihan SPP Bulanan</button>
    ...
    {{-- Bukti SPP menunggu verifikasi (T5.6h) --}}
    <div x-show="tab === 'bukti'" x-cloak class="bg-white border border-slate-100 rounded-[2rem] p-6 md:p-8 shadow-xs space-y-6">
        <h3 class="text-base font-bold text-slate-950">Bukti SPP Menunggu Verifikasi</h3>
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    @empty
                        <tr><td colspan="7" class="py-6 text-center text-slate-400">Tidak ada bukti SPP menunggu verifikasi.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    {{-- Rekap tagihan SPP bulanan (K10.2: rekap global + filter bulan/tahun) --}}
    <div x-show="tab === 'tagihan'" x-cloak class="bg-white border border-slate-100 rounded-[2rem] p-6 md:p-8 shadow-xs space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <h3 class="text-base font-bold text-slate-950">Tagihan SPP Seluruh Murid</h3>

            {{-- Filter bulan/tahun/kelas + search. GET + tab=tagihan agar posisi tab bertahan (rules §6.1). --}}
            <form method="GET" action="{{ route('admin.spp') }}" class="flex flex-wrap items-center gap-2">
                ...
                <select name="id_class" onchange="this.form.submit()"
                        class="border border-slate-200 rounded-lg px-3 py-2 text-xs font-semibold text-slate-600">
                    <option value="">Semua Kelas</option>
                    <option value="none" @selected($kelas === 'none')>Belum Dapat Kelas</option>
                ...
                <select name="status" onchange="this.form.submit()"
                    ...
                    <option value="kurang" @selected($statusFilter === 'kurang')>Kurang</option>
                    <option value="belum_lunas" @selected($statusFilter === 'belum_lunas')>Belum Bayar</option>
                </select>
                <select name="bulan" onchange="this.form.submit()"
                        class="border border-slate-200 rounded-lg px-3 py-2 text-xs font-semibold text-slate-600">
                    <option value="">Semua Bulan</option>
                    @foreach ($bulanOpsi as $bl)
                        <option value="{{ $bl }}" @selected($bulan === $bl)>{{ \Carbon\Carbon::createFromFormat('Y-m', $bl)->translatedFormat('F Y') }}</option>
                    @endforeach
                </select>
                ...
                @if ($bulan || $tahun || $kelas || $statusFilter || $cari)
                    <a href="{{ route('admin.spp', ['tab' => 'tagihan']) }}" class="text-4xs font-bold text-slate-400 hover:text-rose-500 px-2">Reset</a>
                @endif
            </form>
        </div>
        <div class="overflow-x-auto">
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse ($bills as $b)
                        ...
                        <tr class="text-slate-600 hover:bg-slate-50 transition-colors">
                            ...
                            <td class="py-3 text-slate-400">{{ \Carbon\Carbon::createFromFormat('Y-m', $b->bulan)->translatedFormat('F Y') }}</td>
                    ...
                    @empty
                        <tr><td colspan="12" class="py-6 text-center text-slate-400">Belum ada tagihan SPP.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
