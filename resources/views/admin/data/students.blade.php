            ...
            <table class="w-full text-left text-xs border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse ($students as $student)
                        @php
                            $stMap = ['calon' => 'Calon', 'aktif' => 'Aktif', 'alumni' => 'Alumni', 'nonaktif' => 'Nonaktif'];
                            $stClass = ['calon' => 'bg-amber-100 text-amber-700', 'aktif' => 'bg-emerald-100 text-emerald-700', 'alumni' => 'bg-sky-100 text-sky-700', 'nonaktif' => 'bg-slate-100 text-slate-500'][$student->status] ?? 'bg-slate-100 text-slate-500';
                        @endphp
                        <tr class="text-slate-600 hover:bg-slate-50 transition-colors">
                            ...
                            <td class="py-4">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-4xs font-bold uppercase tracking-wider {{ $stClass }}">{{ $stMap[$student->status] ?? $student->status ?? '-' }}</span>
                            </td>
                            <td class="py-4 text-right whitespace-nowrap">
                                @php
                                    $row = [
                                        'id' => $student->id_students, 'slug' => $student->slug, 'nama_lengkap' => $student->nama_lengkap,
                                ...
                                @unless ($student->ortu?->user)
                                    <button @click="openWali('{{ $student->slug }}')" class="px-2.5 py-1.5 border border-emerald-200 hover:bg-emerald-50 text-emerald-600 font-bold rounded-lg">+ Akun Orang Tua</button>
                                @endunless
...
<script>
    ...
    function studentCrud() {
        ...
        return {
            ...
            openEdit(s) {
                this.mode = 'edit';
                this.action = '/portal/admin/data/students/' + s.slug;
            ...
            openWali(slug) {
                this.waliAction = '/portal/admin/data/students/' + slug + '/ortu-account';
                this.waliOpen = true;
            },
        };
    }
